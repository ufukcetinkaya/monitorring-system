...
app.post("/api/ping-results", async (c) => {
  ...
  const item = {
    ...
    checkedAt: body.checkedAt ?? (/* @__PURE__ */ new Date()).toISOString(),
    message: body.message?.trim() || (status === "up" ? "Erişilebilir" : "Erişilemiyor")
  };
...
app.post("/api/ping", async (c) => {
  ...
  const deviceAddress = body.deviceAddress?.trim() || body.targetUrl?.trim() || "1.1.1.1/cdn-cgi/trace";
  const normalizedAddress = /^https?:\/\//i.test(deviceAddress) ? deviceAddress : `http://${deviceAddress}`;
  let parsedUrl;
  try {
